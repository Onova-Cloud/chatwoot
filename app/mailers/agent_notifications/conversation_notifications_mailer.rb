class AgentNotifications::ConversationNotificationsMailer < ApplicationMailer
  def conversation_creation(conversation, agent, _user)
    ...
    subject = "#{@agent.available_name}, una nuova conversazione [ID - #{@conversation.display_id}] è stata creata in #{inbox_name}."
  ...
  def conversation_assignment(conversation, agent, _user)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, una nuova conversazione [ID - #{@conversation.display_id}] ti è stata assegnata."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def conversation_mention(conversation, agent, message)
    ...
    @conversation = conversation
    @message = message
    subject = "#{@agent.available_name}, sei stato menzionato nella conversazione [ID - #{@conversation.display_id}]"
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def assigned_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, nuovo messaggio nella conversazione assegnata [ID - #{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def participating_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, nuovo messaggio nella conversazione a cui partecipi [ID - #{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
